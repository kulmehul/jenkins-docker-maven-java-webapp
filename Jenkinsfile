pipeline {
    ...
    stages {
        stage('SCM') {
            steps {
                git 'https://github.com/kulmehul/jenkins-docker-maven-java-webapp.git'
        ...
        stage('Build Docker OWN image') {
            steps {
                sh "sudo docker build -t  kulmehul/javaweb:${BUILD_TAG}  ."
                //sh 'whoami'
            }
    ...
    // some block
                 sh "sudo docker login -u kulmehul -p $DOCKER_HUB_PASS_CODE"
}
               
               sh "sudo docker push kulmehul/javaweb:${BUILD_TAG}"
            }
        ...
        stage('Deploy webAPP in DEV Env') {
            steps {
                ...
                sh "sudo docker run  -d  -p  8080:8080 --name myjavaapp   kulmehul/javaweb:${BUILD_TAG}"
                //sh 'whoami'
            }
        ...
        stage('Deploy webAPP in Prod Env') {
            steps {
               ...
               sshagent(['QA_ENV_SSH_CRED']) {
    			sh "ssh -o StrictHostKeyChecking=no ec2-user@52.66.207.103 sudo docker rm -f myjavaapp"
                sh "ssh ec2-user@52.66.207.103 sudo docker run -d -p 8080:8080 --name myjavaapp kulmehul/javaweb:${BUILD_TAG}"
                }
